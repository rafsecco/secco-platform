...
|---|---|---|
| [Secco.SharedKernel](src/SharedKernel/Secco.SharedKernel/README.md) | Primitivas compartilhadas (Result, paginação, entidades base, claims/permissions) | `Secco.SharedKernel` **0.4.0** |
| [Secco.SDK.AspNetCore](src/SDK/Secco.SDK.AspNetCore/README.md) | Cross-cutting de runtime (auth, correlation, tenancy, health, resiliência, autorização, OpenAPI) | `Secco.SDK.AspNetCore` **0.8.3** |
...
| [Secco.SDK.ClientCredentials](src/SDK/Secco.SDK.ClientCredentials/README.md) | Client credentials OAuth 2 entre produtos (token de máquina, pacote fino) | `Secco.SDK.ClientCredentials` **0.1.4** |
...
| [Secco.SecureGate](src/SecureGate/README.md) | Identity & Access Management: OIDC (client credentials + login de usuário + federação Entra ID), catálogo de tenants, autorização Role+Permission | Disponível · client `Secco.SecureGate.Client` **0.11.0** |
