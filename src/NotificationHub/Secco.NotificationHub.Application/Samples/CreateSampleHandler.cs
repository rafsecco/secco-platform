...
/// </summary>
public sealed class CreateSampleHandler(ISampleRepository repository, NotificationHubOptions options)
{
    ...
    public async Task<Result<SampleDto>> HandleAsync(CreateSampleCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            return NotificationHubErrors.Samples.NameRequired;
        }

        if (command.Name.Length > options.MaxNameLength)
        {
            return NotificationHubErrors.Samples.NameTooLong(options.MaxNameLength);
        }
