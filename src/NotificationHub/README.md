...
- **Dois providers de e-mail** (issue #14): `Smtp` (MailKit) é o default e atende instalação on-premise; `SendGrid` atende quem sobe em nuvem sem servidor SMTP. O que troca é **apenas** a implementação de `IEmailSender` — a porta já existia desde a Fase 8; job, retry e status são idênticos entre um e outro. Configuração pela metade falha no startup, não no primeiro envio.
