...
- **Canais externos de comunicação corporativa** (issue #13, **ADR-0029**): `teams` e `slack` entram como canais **nativos**, cada um com provider e tradução própria — Adaptive Card no Teams, Block Kit no Slack. O conjunto de canais **continua fechado**: canal novo exige provider no produto e ADR. Não existe canal `webhook` genérico e **a URL nunca vem no payload** — o consumidor pede um canal, o Hub resolve o destino a partir da configuração do tenant. O contrário seria um proxy HTTP dirigido pelo consumidor, SSRF por construção (ADR-0020).
  - **Teams via Workflows (Power Automate).** Não é escolha: os Office 365 Connectors foram desativados entre 18 e 22 de maio de 2026 e webhooks de connector deixaram de funcionar. **Requisito operacional:** a URL do Workflow pertence a uma pessoa e fica órfã se ela sair da organização — configure coproprietário. Mensagens aparecem com a identidade do Flow bot.
  - **Slack via incoming webhook de um Slack app.** A documentação oficial não marca como descontinuado; `chat.postMessage` só é recomendado para apagar, editar ou rotear dinamicamente — se isso virar necessidade, o caminho passa a ser bot token.
  - **Em lote, canal externo entrega uma vez, não N.** Uma publicação que alcança 500 pessoas por e-mail posta **uma** mensagem no canal da empresa.
- **Dois providers de e-mail** (issue #14, promovido ao `Secco.SDK.Email` na ADR-0033): `Smtp` (MailKit) é o default e atende instalação on-premise; `SendGrid` atende quem sobe em nuvem sem servidor SMTP. O que troca é **apenas** a implementação de `ISeccoEmailSender` — a porta já existia desde a Fase 8; job, retry e status são idênticos entre um e outro. Configuração pela metade falha no startup, não no primeiro envio.
- **Despacho em lote** (issue #15): `POST /api/v1/notifications/batch` leva **um conteúdo para muitos destinos** numa chamada só — o caso de uma publicação que precisa alcançar um público inteiro. Repetir título e mensagem por destinatário mandaria o mesmo texto centenas de vezes pela rede; conteúdo diferente por pessoa continua sendo o endpoint unitário. Validação é **tudo-ou-nada** e aponta a posição do destino inválido: gravar os válidos e reportar o resto deixaria o chamador sem saber o que repetir. As notificações vão ao banco numa ida só, mas o enfileiramento segue sendo um job por destino — é o que preserva o retry por notificação, e é a razão do teto `NotificationHub:Limits:MaxBatchDestinations` (default 500).
...
| `NotificationHub:Email` | `Provider` (`Smtp` padrão / `SendGrid`), `FromAddress`/`FromName` sempre; `Host`/`Port`/`UseStartTls`/`Username`/`Password` para SMTP; `ApiKey` para SendGrid. **Validado no startup**: só o provider selecionado tem chaves exigidas |
| `NotificationHub:Channels` | Nada aqui: o destino de `teams` e `slack` é **por tenant**, guardado cifrado no banco do próprio tenant e gerido por `/api/v1/channel-configurations` (ADR-0029). Configuração por instalação foi descartada — numa instalação multi-tenant, destino compartilhado faria notificação de um tenant chegar ao canal de outro |
