...
/// <para>
/// Usa o mesmo caso de uso da redefinição, com <c>invite: true</c>: revogação de sessão e trilha
/// não podem divergir entre os dois caminhos só porque as telas são duas.
/// </para>
/// <para>
...
public sealed class SetPasswordModel(ResetPasswordHandler handler, ICredentialTokens tokens) : PageModel
{
	...
	public async Task<IActionResult> OnPostAsync(CancellationToken cancellationToken)
	{
		...
		var outcome = await handler
			.HandleAsync(UserId, Token, invite: true, Input.Password, cancellationToken)
			.ConfigureAwait(false);
