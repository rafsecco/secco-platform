...
1. **Mover os testes** para a raiz de testes do monorepo e ajustar o caminho do `ProjectReference` no csproj:
   `git mv src/SecureGate/tests tests/SecureGate`
2. **Gerar as migrations iniciais** (uma por engine, ADR-0018):
   ```bash
   dotnet ef migrations add Initial --project src/SecureGate/Secco.SecureGate.Migrations.SqlServer --output-dir Migrations
   dotnet ef migrations add Initial --project src/SecureGate/Secco.SecureGate.Migrations.Postgres --output-dir Migrations
   ```
3. **Adicionar os projetos à solution**: `dotnet sln Secco.Platform.slnx add src/SecureGate/**/*.csproj tests/SecureGate/**/*.csproj`
