...
public class OpenApiContractTests(SampleServiceApiFactory factory) : IClassFixture<SampleServiceApiFactory>
{
    ...
    {
        // Sobe até a pasta do produto (a que contém Secco.SampleService.Api)
        var directory = new DirectoryInfo(AppContext.BaseDirectory);

        while (directory is not null && !Directory.Exists(Path.Combine(directory.FullName, "Secco.SampleService.Api")))
        {
