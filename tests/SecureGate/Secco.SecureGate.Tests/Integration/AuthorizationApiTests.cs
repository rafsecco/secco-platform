...
public class AuthorizationApiTests(SecureGateApiFactory factory) : IAsyncLifetime
{
	...
	[Fact]
	public async Task GetPermissions_ForInstallationOperator_ReturnsReadSetForAnyTenant()
	{
		var reader = CreateClientWithScopes(SecureGateScopes.AuthorizationRead);

		// Um tenant aleatório onde o role de operador nunca foi cadastrado (ADR-0024)
		var permissions = await reader.GetFromJsonAsync<string[]>(
		...
		permissions.Should().BeEquivalentTo(SecureGatePlatform.OperatorReadPermissions,
			"o operador de instalação recebe o read-set em qualquer tenant, resolvido no SecureGate");
