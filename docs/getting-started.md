...
|---|---|---|
| `Secco.SharedKernel` | 0.3.7 | `Result<T>`, paginação, entidades base, claims/permissions |
| `Secco.SDK.AspNetCore` | 0.7.0 | Cross-cutting de runtime (auth, tenancy, correlation, health, resiliência, autorização, OpenAPI, background jobs) |
...
| `Secco.SecureGate.Client` | 0.4.0 | Client do SecureGate + `ITenantCatalog`/`IPermissionResolver` prontos |
| `Secco.LogStream.Client` | 0.4.0 | Client do LogStream |
| `Secco.SDK.ClientCredentials` | 0.1.0 | Client credentials OAuth 2 entre produtos; os clients de produto o usam sozinhos |
